...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($id_cliente) || empty($nombre) || empty($apellido) || empty($email)) {
        die("Error: Faltan datos obligatorios para la actualización.");
    }
    ...
    if ($stmt->execute()) {
        $message = urlencode('Cliente actualizado exitosamente.');
        header("Location: ../../public/admin/clientes.php?status=success&message=" . $message);
    } else {
        $message = urlencode('Error al actualizar el cliente: ' . $stmt->error);
