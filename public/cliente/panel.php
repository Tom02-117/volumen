...
// --- Lógica para calcular días restantes y progreso ---
$dias_restantes = 0;
...
// --- Lógica para las Clases Reservadas ---
$sql_clases = "
...
<head>
    ...
    <!-- Vincularemos un nuevo archivo CSS para el panel -->
...
<body>
    ...
    Swal.fire({
        ...
        text: "Tu nueva membresía ha sido activada. ¡A entrenar!",
        confirmButtonColor: "#28a745"
    }).then(() => {
        // Limpia la URL para que el mensaje no vuelva a salir si se recarga
        window.history.replaceState({}, document.title, window.location.pathname);
    });
}
      // Mantenemos la lógica de SweetAlert que ya tenías.
      const params = new URLSearchParams(window.location.search);
      if (params.get("login") === "success") {
        let nombre = "<?php echo $_SESSION['user_nombre']; ?>"; 
        Swal.fire({
          icon: "success",
      ...
      document.querySelectorAll(".form-cancelar").forEach(form => {
        form.addEventListener("submit", function(e) {
          e.preventDefault(); 
          let button = form.querySelector(".btn-cancelar");
          let clase = button.dataset.clase;
          let dia = button.dataset.dia;
          let hora = button.dataset.hora;

          Swal.fire({
            title: "¿Seguro que quieres cancelar?",
            text: `Clase: ${clase} (${dia} - ${hora}). Esta acción no se puede deshacer.`,
            icon: "warning",
      ...
      if (params.get("cancel") === "error") {
        Swal.fire({
          icon: "error",
          title: "Error",
          text: "No se pudo cancelar la clase. Inténtalo de nuevo.",
          confirmButtonColor: "#d33"
        }).then(() => {
          window.history.replaceState({}, document.title, window.location.pathname);
        });
